...
exports.allData = async (req, res) => {
    ...
    try {
        ...
        if (skipNum < count1) {
            ...
            let data = await PostalAuction.find(query).skip(skipNum).limit(limitNum).select('FILENAME CUSTOMER_NAME NOTICE_URL BAR_CODE MOBILE_NUMBER');
            ...
            let data = await PostalDeficit.find(query).skip(skipNum - count1).limit(limitNum).select('FILENAME CUSTOMER_NAME NOTICE_URL BAR_CODE MOBILE_NUMBER');
            let count = count1+count2+count3;
            ...
            let data = await PostalRefund.find(query).skip((skipNum - count1) - count2).limit(limitNum).select('FILENAME CUSTOMER_NAME NOTICE_URL BAR_CODE MOBILE_NUMBER');
            let count = count1+count2+count3;
