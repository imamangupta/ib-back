...
exports.addPost = async (req, res) => {
    ...
    upload.single("excelFile")(req, res, async (err) => {
        ...
        try {
            ...
            const post = await SmsMtm.insertMany(result)
...
exports.fAll = async (req, res) => {
    ...
    try {
        ...
        let count = await SmsMtm.countDocuments(query)
        let data = await SmsMtm.find(query).skip(skipNum).limit(limitNum).select('-date -CITY -STATE -DATE -__v -_id');
